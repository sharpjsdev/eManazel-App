...
@Component({
  selector: 'app-agreemets',
  templateUrl: './agreemets.page.html',
  styleUrls: ['./agreemets.page.scss'],
})
export class AgreemetsPage implements OnInit {
  language: string;
  user_type: string;
  community_id: string;
  user_id: string;
  ...
  ionViewWillEnter() {
    ...
    this.authService.common('getServiceProviderAgreement',meeting_post).subscribe((res) => {
      if(res['success'] == 1){
        ...
        meeting_post.append('agreement_id', this.agreement_id);
        this.authService.common('getAllAgreementOfServiceProvider',meeting_post).subscribe((res) => {
          if(res['success']=='1'){

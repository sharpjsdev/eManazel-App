...
export class AddServiceProviderPage implements OnInit {
  ...
  ionViewWillEnter() {
    ...
    this.authService.common('getCommunityItemServiceType',dept).subscribe((res) => {
      if(res['success']=='1'){
        this.items = res['data'];
      }
    })
  ...
  userForm(form){
    ...
    meeting_post.append('item_type', form.value.item_id);        
    ...
    meeting_post.append('service_image',imge);      
    this.authService.common('addServiceProvider',meeting_post).subscribe((res) => {
      if(res['success']=='1'){

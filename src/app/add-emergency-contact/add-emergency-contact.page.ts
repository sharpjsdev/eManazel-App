...
@Component({
  ...
  templateUrl: './add-emergency-contact.page.html',
  styleUrls: ['./add-emergency-contact.page.scss'],
})
export class AddEmergencyContactPage implements OnInit {
  app_url: string;
  user_type: string;
  user_id: string;
  community_id: string;
  ...
  save(form){
    ...
    this.authService.common('addEmergencyContact',meeting_post).subscribe((res) => {
      if(res['success']=='1'){
